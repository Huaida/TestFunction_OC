...
// set navigationBar barTintColor  第一次 把一个背景图添加到navigationBar最上面
...
static char kWRPushToNextVCFinishedKey;// push到了下一个的VC了
static char kWRNavBarBackgroundImageKey;//
static char kWRNavBarBarTintColorKey;//  导航栏的颜色
static char kWRNavBarBackgroundAlphaKey;//
static char kWRNavBarTintColorKey;//  左右按钮的颜色
static char kWRNavBarTitleColorKey;// title的颜色
static char kWRStatusBarStyleKey;//  状态栏样式
static char kWRNavBarShadowImageHiddenKey;//
static char kWRCustomNavBarKey;// 自定义导航栏
static char kWRSystemNavBarBarTintColorKey;// 系统导航栏的条
...
// custom navigationBar  获取绑定的navibar
...
// 进入页面时，如果对黑名单VC导航栏设置复原
- (void)wr_viewWillAppear:(BOOL)animated {
    if ([self canUpdateNavigationBar]) { // 能够更新导航栏
        if (![WRNavigationBar needUpdateNavigationBar:self]) { //不需要更新导航栏
            if ([self wr_systemNavBarBarTintColor] == nil) { // 对黑名单中的控制导航栏颜色复原
